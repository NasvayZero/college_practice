...
# TODO: потом подумать, куда лучше файлы сохранять
UPLOAD_DIR = "uploads"
# на всякий случай, если сообщение будет очень длинным
MAX_MSG_LENGTH = 4000


async def start(update, context):
    """Приветствие - работает вроде норм"""
...
async def send_long_message(update, text, chunk_size=MAX_MSG_LENGTH):
    """Разбиваем длинные отчеты на части - иначе телеграм ругается"""
    # можно было бы через \n разбивать, но пока так
    for i in range(0, len(text), chunk_size):
        chunk = text[i:i + chunk_size]
        # Костыль на случай, если обрезали посередине слова - не критично
...
async def handle_document(update, context):
    ...
    elif "посещаемост" in file_name:  # посещаемости, посещаемость
    ...
    # TODO: удалить файл после обработки? Или оставить для отладки
...
def main():
    ...
    try:
        ...
        # Бот реагирует только на документы
